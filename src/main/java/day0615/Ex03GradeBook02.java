...
public class Ex03GradeBook02 {
    ...
    // 전역 상수 선언
    // 1. 과목수 선언(SUBJECT_SIZE)
    public static final int SUBJECT_SIZE = 3;
    // 2. 최소 점수 선언(SCORE_MIN)
    public static final int SCORE_MIN = 0;
    // 3. 최대 점수 선언(SCORE_MAX)
    ...
    public static void showMenu() {
        // 프로그램은 종료를 입력받을때까지 반복
        while(true) {
            // 사용자의 메뉴선택 입력부
            String msg = "프로그램을 실행합니다.\n메뉴를 골라주세요.\n1. 학생정보 입력  2. 학생목록 출력  3. 종료";
            int userChoice = ScannerUtil.nextInt(scanner, msg);

            // 사용자가 학생정보 입력을 선택하였을 경우
            if(userChoice == 1) {
                // 학생정보 입력 메소드 insert() 호출
                insert();
            }else if(userChoice == 2) {
                // 학생정보 출력 메소드 printAll() 호출
                printAll();
    ...
    public static void insert() {
        ...
        s.kor = ScannerUtil.nextInt(scanner, msg, SCORE_MIN, SCORE_MAX);

        // 학생의 영어 점수를 입력받는다.
        msg = "학생의 영어 점수를 입력해주세요.";
        s.eng = ScannerUtil.nextInt(scanner, msg, SCORE_MIN, SCORE_MAX);
    ...
    public static void printAll() {
        ...
        if(!ArrayUtil.isEmpty(arr)) {
            // 간략한 정보들을 출력해준다.(id. name)
            for(Student s : arr) {
                System.out.printf("%d. %s\n", s.id, s.name);
            }

            // 상세보기 할 id값을 입력받는다.
            String msg = "상세보기할 학생의 번호를 입력해주세요.(0 입력시 뒤로가기)";
            int userChoice = ScannerUtil.nextInt(scanner, msg);
